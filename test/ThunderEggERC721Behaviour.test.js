...
contract('Core ERC721 behaviour tests for ThunderEgg', function ([owner, approved, anotherApproved, operator, other, buyer]) {

    const ONE_THOUSAND_TOKENS = to18DP('1000');

    const name = 'ThunderEgg';
    const symbol = 'TEGG';
    ...
    beforeEach(async function () {
        this.lava = await LavaToken.new(to18DP('1000'), owner, owner, {from: owner});

        this.stakingToken = await MockERC20.new('LPToken', 'LP', ONE_THOUSAND_TOKENS.mul(new BN('4')), {from: owner});
        await this.stakingToken.transfer(owner, ONE_THOUSAND_TOKENS, {from: owner});

        this.token = await ThunderEgg.new(
          this.lava.address,
          to18DP('1'),
          new BN('0'),
          new BN('0'), // bonus block
          {from: owner}
        );

        await this.lava.changeMinter(this.token.address, {from: owner});

        (await this.token.lavaPerBlock()).should.be.bignumber.equal(to18DP('1'));
        (await this.token.startBlock()).should.be.bignumber.equal('0');

        await this.token.addSacredGrove(new BN('1'), this.stakingToken.address, true, {from: owner});

        this.groveId = (await this.token.sacredGroveLength()).sub(new BN('1'));

        await this.stakingToken.approve(this.token.address, ONE_THOUSAND_TOKENS, {from: owner});
    });
    ...
    describe('metadata', function () {
        ...
        describe('token URI', function () {
            beforeEach(async function () {
                await this.token.spawn(this.groveId, ONE_THOUSAND_TOKENS, '0x0', {from: owner});
            });
        ...
        describe('ownerOf', function () {
            ...
            context('when the given token ID was not tracked by this token', function () {
                ...
                it('reverts', async function () {
                    await expectRevert(
                        this.token.ownerOf(tokenId), 'ERC721: operator query for nonexistent token',
                    );
                });
            });
        });
    ...
    describe('_mint(address, uint256)', function () {
        context('with minted token', async function () {
            beforeEach(async function () {
                ({ logs: this.logs } = await this.token.spawn(this.groveId, ONE_THOUSAND_TOKENS, '0x0', {from: owner}));
            });
